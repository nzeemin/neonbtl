    ...
    LPCTSTR sFileName)
{
    ...
    bih.biWidth = NEON_SCREEN_WIDTH;
    bih.biHeight = NEON_SCREEN_HEIGHT;
...
uint32_t ReadValueMSB(const BYTE * buffer)
{
...
void SaveValueMSB(BYTE * buffer, uint32_t value)
{
...
bool PngFile_WriteHeader(FILE * fpFile, BYTE bitdepth)
{
    ...
    SaveValueMSB(IHDRchunk + 8, NEON_SCREEN_WIDTH);
    SaveValueMSB(IHDRchunk + 12, NEON_SCREEN_HEIGHT);
...
bool PngFile_WriteImageData8(FILE * fpFile, uint32_t framenum, const uint32_t* pBits, const uint32_t* palette)
{
    ...
    uint32_t pDataLength = 8 + 2 + (6 + NEON_SCREEN_WIDTH) * NEON_SCREEN_HEIGHT + 4/*adler*/ + 4;
...
bool PngFile_WriteFctl(FILE * fpFile, uint32_t framenum)
{
    ...
    SaveValueMSB(acTLchunk + 8 + 4, NEON_SCREEN_WIDTH);
    SaveValueMSB(acTLchunk + 8 + 8, NEON_SCREEN_HEIGHT);
